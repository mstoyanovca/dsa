...
public class NumberOfPairs {
    private final int[] exceptions = new int[4];
    ...
    public long countPairs(int[] x, int[] y, int m, int n) {
        ...
        /*for (int i = 0; i < m; i++) {
            for (int j = 0; j < n; j++) {
                if (Math.pow(x[i], y[j]) > Math.pow(y[j], x[i])) counter++;
            }
        }*/

        for (int i = 0; i < m; i++) {
            counter += count(x[i], y, n);
        }

        return counter;
    }

    private int count(int x, int[] y, int n) {
        if (x == 1) return 0;
        int index = nextGreaterElementIndex(x, y, n);
        int count = index >= 0 ? n - index : 0;

        count += exceptions[0];  // ones
        if (x == 2) count -= exceptions[2] + exceptions[3];  // threes + fours
        if (x == 3) count += exceptions[1];  // twos
